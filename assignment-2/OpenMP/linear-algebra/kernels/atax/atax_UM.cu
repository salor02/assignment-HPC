                 ...
                 DATA_TYPE POLYBENCH_1D(tmp, NX, nx))
{
    ...
    cudaMallocManaged(&d_A, sizeof(DATA_TYPE) * NX * NY);
    cudaMallocManaged(&d_x, sizeof(DATA_TYPE) * NY);
    cudaMallocManaged(&d_y, sizeof(DATA_TYPE) * NY);
    cudaMallocManaged(&d_tmp, sizeof(DATA_TYPE) * NX);

    // Copy data from host to Unified Memory
    memcpy(d_A, A, sizeof(DATA_TYPE) * NX * NY);
    memcpy(d_x, x, sizeof(DATA_TYPE) * NY);
    memcpy(d_y, y, sizeof(DATA_TYPE) * NY);
    memcpy(d_tmp, tmp, sizeof(DATA_TYPE) * NX);
    ...
    // Unified memory for the transposed matrix
    DATA_TYPE *d_A_T;
    cudaMallocManaged(&d_A_T, sizeof(DATA_TYPE) * NX * NY);
    ...
    // Copy data back to host (optional, as Unified Memory is accessible from CPU)
    memcpy(y, d_y, sizeof(DATA_TYPE) * NX);
